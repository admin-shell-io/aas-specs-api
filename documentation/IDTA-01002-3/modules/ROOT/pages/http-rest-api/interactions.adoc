...
====
Note: If the AAS or AAS Repository Service is used to get the submodel IDs it is recommended that the data provider adds a link:https://industrialdigitaltwin.io/aas-specifications/IDTA-01001/v3.2/spec-metamodel/referencing.html#Reference[referredSemanticId] to the references of the submodels.
...
[plantuml, seq-sm-endpoints-http, svg]
....
include::partial$diagrams/seq-sm-endpoints-http.puml[]
....

== Accessing recently changes objects

For recently changed Asset Administration Shells or Submodels there are the two API Operations 
xref:specification/interfaces-payload.adoc#GetAllSubmodelsRecentChanges[GetAllSubmodelsRecentChanges] and 
xref:specification/interfaces-payload.adoc#GetAllAssetAdministrationShellsRecentChanges[GetAllAssetAdministrationShellsRecentChanges].

For AAS Registry and Submodel Registry recently changed Asset Administration Shells or Submodels corresponding date input parameters are directly added to   
xref:specification/interfaces-payload.adoc#GetAllAssetAdministrationShellDecriptors[GetAllAssetAdministrationShellDescriptors] and
xref:specification/interfaces-payload.adoc#GetAllSubmodelDecriptors[GetAllSubmodelDescriptors]. 

In <<fig:seq-repo-recent-changes>> a sequence interaction is described for an application looking for Asset Administration Shells created after a specified date interacting directly with an AAS repository.
In <<fig:seq-registry-recent-changes>> a sequence interaction is described for an application looking for Asset Administration Shells created after a specified date but using the AAS Registry as entry point.



.Interaction for Client Application looking for Recent Changes using an Asset Administration Shell Repository 
[[fig:seq-repo-recent-changes]]
[plantuml, seq-repo-recent-changes, svg]
....
include::partial$diagrams/seq-repo-recent-changes.puml[]
....


.Interaction for Client Application looking for Recent Changes using an Asset Administration Shell Registry
[[fig:seq-registry-recent-changes]]
[plantuml, seq-registry-recent-changes, svg]
....
include::partial$diagrams/seq-registry-recent-changes.puml[]
....
