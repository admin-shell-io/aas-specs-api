...
Nevertheless, AAS servers may decide to use different paths depending on their context, see Figure <<aas-api-versioning-url-scheme2>>. A client shall not assume that the pattern from Figure <<aas-api-versioning-url-scheme>> is supported by all servers.
