...
The OpenAPI specification of the HTTP/REST APIs can be found https://industrialdigitaltwin.io/aas-specs-api/docs/swagger-ui.html[here] and, for legacy reasons, also at https://app.swaggerhub.com/search?owner=Plattform_i40[SwaggerHub] until version v3.1.3.
...
https://industrialdigitaltwin.io/aas-specs-api/docs/swagger-ui.html?url=./Part1-MetaModel-Schemas/openapi_3.2.yaml&version=v3.2.0[https://industrialdigitaltwin.io/aas-specs-api/docs/swagger-ui.html?url=./Part1-MetaModel-Schemas/openapi_3.2.yaml&version=v3.2.0] +
...
https://industrialdigitaltwin.io/aas-specs-api/docs/swagger-ui.html?url=./Part2-API-Schemas/openapi.yaml&version=v3.2.0[https://industrialdigitaltwin.io/aas-specs-api/docs/swagger-ui.html?url=./Part2-API-Schemas/openapi.yaml&version=v3.2.0] +
This schema includes general objects, which are used in the further defined APIs.

The specific OpenAPI documents for the different APIs are listed in the sections for the respective service specifications and profiles in Clause xref:http-rest-api/service-specifications-and-profiles.adoc#service-specifications-and-profiles[Service Specifications and Profiles].
...
* OpenAPI, hosted by SwaggerHub (legacy versions) and rendered via Github Pages shall be used for specification.
...
In particular, base64url-encoded values shall not contain or end with the `=` character (so-called 'padding') to keep them URL-safe.
...
The link:https://industrialdigitaltwin.io/aas-specifications/IDTA-01001/v3.2/mappings/mappings.html#value-only-serialization-in-json[serialization of JSON values] is described in xref:bibliography.adoc#bib1[[1]].
...
|GetThumbnail |GET |/aas/asset-information/thumbnail |See <<retrieving-files>> for file content and external thumbnail redirects.
...
See <<retrieving-files>> for file content and external file redirects.
...
|GetAssetAdministrationShellVersionByIdAndDate |GET |/shells/\{aasIdentifier}/$history a|
base64url-encoded identifier
...
|GetAllSubmodelsRecentChanges |GET |/submodels/$recent-changes a|
...
|GetSubmodelVersionByIdAndDate |GET |/submodels/\{submodelIdentifier}/$history a|
base64url-encoded identifier
...
|GetAllAssetAdministrationShellRecentChanges |GET |/shells/$recent-changes a| _key_ and _keyIdentifier_ are mapped to base64url-encoded JSON-serialized key-value-pairs
...
|GetAllAssetAdministrationShellDescriptors |GET |/shell-descriptors a| _key_ and _keyIdentifier_ are mapped to base64url-encoded JSON-serialized key-value-pairs
...
|Submodel Registry Interface s|* s|/shell-descriptors/\{aasIdentifier}/submodelDescriptors/* s|superpath as defined in Service Specification or Profile
...
|GetAllSubmodelDescriptors |GET |/submodel-descriptors a| 
...
[#retrieving-files]
== Retrieving Files and Thumbnails

Clients can use `GetFileByPath` (`GET .../submodel-elements/\{idShortPath}/attachment`) to retrieve the file associated with a File SubmodelElement, and `GetThumbnail` (`GET .../asset-information/thumbnail`) to retrieve the default thumbnail of an asset.
These rules apply to every service specification and profile offering these operations, including the corresponding repository and nested Submodel endpoints.
The `idShortPath` identifies the File SubmodelElement within the Submodel; it is not the file's location.
The file's location is given by `File.value`, or by `AssetInformation.defaultThumbnail.path` for a thumbnail.

After applying the authorization checks for the requested operation, the server shall handle the location as follows:

* If the location is an absolute URL with the scheme `http` or `https`, the server shall return `302 Found` with that URL in the `Location` response header, as defined in https://www.rfc-editor.org/rfc/rfc9110.html#section-15.4.3[RFC 9110, Section 15.4.3].
The response does not contain the file content.
The server shall not fetch the external resource or make an outbound request to check its availability while handling this operation.
* Otherwise, the server shall return the file content it manages for the requested File SubmodelElement or thumbnail with `200 OK`.
If no location is provided or no corresponding managed file is available, the server shall return `404 Not Found`.
The server shall resolve other URI schemes and relative paths only against its managed file storage.
It shall not dereference them as arbitrary filesystem paths or network locations, or return them as external redirect destinations.

A `302 Found` response indicates where the client can request the resource; it does not guarantee that the external resource exists or that the client is authorized to access it.
The client can follow the redirect according to its own security policy without first retrieving and interpreting `File.value` or the thumbnail's `path`.
Authorization for the attachment or thumbnail operation shall be enforced before disclosing the redirect URL.
Access to the external resource is subject to the external server's own authorization requirements.

====
Note: Fetching a URL supplied through writable model data on behalf of a client can expose the AAS server to server-side request forgery (SSRF), for example by making it contact internal services.
Returning a redirect avoids this server-side fetch.
Clients remain responsible for validating redirect destinations, limiting redirect chains, and protecting their own network access when following redirects.
Credentials intended for the AAS server must not be forwarded to another origin.
====

For example, if the File SubmodelElement `Manual` has `value` set to `https://files.example.com/manual.pdf`, a request to `GET /submodel/submodel-elements/Manual/attachment` returns:

[source,http]
----
HTTP/1.1 302 Found
Location: https://files.example.com/manual.pdf
Content-Length: 0
----
...
|RedirectionFound |The requested resource can be requested at another location |302 (Found) |The Location response header contains the URL where the client can request the resource. For external files and thumbnails, see <<retrieving-files>>.
...
The generic status code `RedirectionFound` maps to `302 Found` in the HTTP/REST binding.
For external files and thumbnails, the required `Location` header and response behavior are defined in <<retrieving-files>>.
A redirect is not an error and does not require a Result object or a File Content payload.
