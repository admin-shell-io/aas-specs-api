...
AAS API versioning uses the major and minor version as described above. A specific AAS API version uses specific related versions of the metamodel as defined in Clause xref:index.adoc#metamodel-versions[Metamodel Versions]. AAS API versions with the same major version must remain compatible, i.e. a client written for an older or a newer minor version must still work. This requires corresponding testing of clients and servers.
...
|QueryAssetAdministrationShells |POST |/query a| Input query in the request body


====
Note: The query for this operation shall not contain any 'select' statement. 
====


|QueryAssetAdministrationShellIds |POST |/query a| Input query in the request body

====
Note: The query for this operation must contain a 'select' statement. 
====
...
|QuerySubmodels |POST |/query a| Input query in the request body
...
|QuerySubmodelIds |POST |/query a| Input query in the request body
...
|QueryConceptDescriptions |POST |/query a| Input query in the request body
...
|QueryConceptDescriptionIds |POST |/query a| Input query in the request body
...
|QueryAssetAdministrationShellDescriptors |POST |/query a| Input query in the request body
...
|QueryAssetAdministrationShellDescriptorIds |POST |/query a| Input query in the request body
...
|QuerySubmodelDescriptors |POST |/query a| Input query in the request body
...
|QuerySubmodelIdDescriptors |POST |/query a| Input query in the request body
...
In the asynchronous pattern, the server immediately responds with an Accepted (status code: 202) message containing the link to an endpoint where the client can fetch status information about his request (see <<se-operation-invocation>>). This status endpoint is also located at the same SubmodelElement “Operation”, followed by the path segments "/operation-status/\{handleId}”.
...
A client executing a bulk request will retrieve the location of the status endpoint through the Location header of the response (see <<sequence-of-bulk>>). As long as the request is processed, the status endpoint responds with “OK” with an optional information for the client when it shall ask again ("Retry-After").
...
The serialised query has to match the grammar as specified in Clause (TODO: Insert reference). If a server retrieves a mal-formed query, it must respond with a ClientErrorBadRequest message. 
...
AAS queries can be sent via POST requests, delivering the serialised query JSON in the request body, to the reserved `/query` endpoint of an AAS service. It is recommended that a server exposes its supported query profiles through the `/description` endpoint.


// TODO: continue with the text
