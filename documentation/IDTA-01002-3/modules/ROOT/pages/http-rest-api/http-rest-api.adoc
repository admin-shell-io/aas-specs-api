...
The serialised query has to match the grammar as specified in Clause xref:index.adoc#query-json-serialisation[Serialisation]. If a server retrieves a mal-formed query, it must respond with a ClientErrorBadRequest message. 
...
[source,json]
----
include::./query-json-schema.json[]
----
