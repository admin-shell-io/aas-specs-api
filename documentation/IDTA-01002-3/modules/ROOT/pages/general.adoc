...
==== Pagination
To manage the pagination of the retrieved data, incorporate the limit and skip operators within the options configuration.

[source,javascript]
----
$limit( <value> ), $skip ( <value> )
----
...
| $skip | A xs:int value that specifies where to start fetching data, also known as offset
| $limit | A xs:int value that indicates how many results to fetch per page
|===
