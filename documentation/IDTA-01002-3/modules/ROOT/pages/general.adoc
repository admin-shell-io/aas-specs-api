...
image::../images/plantuml/query-language-overview.png[image24,width=642]
...
| $aas#id $contains "https://example.com/asset-administration-shell" | true | "https://example.com/asset-administration-shell" is a substring of the `id` value ("https://example.com/asset-administration-shell-1")
|===
...
| $limit | To control the pagination of the retrieved data, use the optional limit operator
| start | A xs:int value that specifies where to start fetching data, also known as offset
| count | A xs:int value that indicates how many results to fetch per page
|===

Example to return the first five elements of the result set.
[source,javascript]
----
$limit( 0, 5 )
----

Example to return five elements, where the first returned element is the sixth of the result set.
[source,javascript]
----
$limit( 5, 5 )
----
...
| direction | "asc" for ascending sort order and "dsc" for descending sort order.
