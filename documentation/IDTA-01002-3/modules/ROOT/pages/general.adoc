...
....
<query> ::= ("select id where ")? ( <queryParameter> <whitespace> )* <queryParameter> <whitespace>* <options>?
 
<queryParameter> ::= <singleComparison> | <logicalOperatorExpression>

<logicalOperatorExpression> ::= 
    ( "not(" <logicalOperatorExpression> ")" ) | 
    ( <logicalOperatorExpression> <whitespace>+ "or" <whitespace>+ <logicalOperatorExpression> ) | 
    ( <logicalOperatorExpression> <whitespace>+ "and" <whitespace>+ <logicalOperatorExpression> ) |
    <singleComparison>

<singleComparison> ::=
    (
    "starts-with" | "ends-with" | "contains"
    "(" <FieldIdentifier> <whitespace>* "," <whitespace>+ ( <StringLiteral> | <identifier> | <FieldIdentifier> ) ")"
    )
    |  
    (
    <FieldIdentifier> 
    <whitespace>
    ( "=" | "!=" | "<" | "<=" | ">" | ">=" **OR** "eq" | "ne" | "gt" | "lt" | "ge" | "le" )
    <whitespace>
    ( <NumericalLiteral> | <StringLiteral> | <identifier> | <FieldIdentifier> )
    )

<options> ::= <sortExpression>? <limitExpression>?

<sortExpression> ::= "sort" "( id" (" asc" | " desc") ")"  // TODO: explain why only 'id' is available at the beginning
...
Updated grammar 2024-09-26

....
<queryParameter> ::= <singleComparison> | <logicalOrExpression>

<logicalOrExpression> ::= 
    <logicalAndExpression> |
    ( <logicalOrExpression> <whitespace>+ "or" <whitespace>+ <logicalAndExpression> ) |
    "(" <logicalOrExpression> ")"

<logicalAndExpression> ::= 
    <logicalNotExpression> |
    ( <logicalAndExpression> <whitespace>+ "and" <whitespace>+ <logicalNotExpression> ) |
    "(" <logicalAndExpression> ")"
...
<stringComparison> ::= 
    ( "starts-with" | "ends-with" | "contains" 
    "(" <stringOperand> <whitespace> "," <whitespace> <stringOperand> ")" ) |
    <stringOperand> <whitespace> ( "eq" | "ne" | "gt" | "lt" | "ge" | "le" ) <whitespace> <stringOperand>

<numericalComparison> ::= 
    <numericalOperand> <whitespace> ( "eq" | "ne" | "gt" | "lt" | "ge" | "le" ) <whitespace> <numericalOperand>

<hexComparison> ::= 
    <hexOperand> <whitespace> ( "eq" | "ne" | "gt" | "lt" | "ge" | "le" ) <whitespace> <hexOperand>

<boolComparison> ::= 
    <boolOperand> <whitespace> ( "eq" | "ne" ) <whitespace> <boolOperand>

<stringOperand> ::= 
    <FieldIdentifierString> | <StringLiteral> | <castToString>

<numericalOperand> ::= 
    <NumericalLiteral> | <castToNumerical>
...
<castToString> ::= 
    "str(" ( <FieldIdentifierString> | <NumericalLiteral> | <HexLiteral> | <BoolLiteral> ) ")"

<castToNumerical> ::= 
    "num(" ( <FieldIdentifierString> | <StringLiteral> | <HexLiteral> | <BoolLiteral> ) ")"

<castToHex> ::= 
    "hex(" ( <FieldIdentifierString> | <StringLiteral> | <NumericalLiteral> | <BoolLiteral> ) ")"

<castToBool> ::= 
    "bool(" ( <FieldIdentifierString> | <StringLiteral> | <NumericalLiteral> | <HexLiteral> ) ")"
...
....
Example:
not(hex("12") gt 16#0ACD) or (hex(sm.idShort) ne 16#ABC and num(sme.value) gt 12) or 12 eq num("12")


Examples (todo: delete before publication):
....
...
TODO: Add examples with logical operators
...
| contains / match | Evaluates a regex expression in the first parameter against the string content of the second. | Defined as fn:contains in https://www.w3.org/TR/xpath-functions-30/#func-contains
| exists | Checks if the argument is a non-empty sequence. | Defined as fn:exists in https://www.w3.org/TR/xpath-functions-30/#func-exists
|===
...
| not | Negates an expression. | not( true ) --> false TODO: Find a proper definition.
|===
...
| "displayName" eq / = 'value' | false | Empty nodelist in the example object, which always evaluates to `false` for `eq` / `=`  __TODO__ continue table adjustments
