...
....
<query> ::= ("select id ")? ( <queryParameter> <whitespace> )* <queryParameter> <whitespace>* <options>?
...
<singleComparison> ::=
    (
    ( "str_eq" | "str_ne" | "starts" | "ends" | "contains" )
    "(" <FieldIdentifier> "," ( <StringLiteral> | <identifier> | <FieldIdentifier> ) ")"
    )
    |  
    ( 
    "num_" ( "eq" | "ne" | "gt"  | "lt" | "ge" | "le" )
    "(" <FieldIdentifier> "," ( <NumericalLiteral> | <identifier> | <FieldIdentifier> ) ")"
    )
    |
    (
...
// filter=not(or(num_eq(1,2),num_eq(10,20),str_ge("a","aa")))
....


=== Grammar Option 2 (closer to XPath / XQuery)

....
<query> ::= ("select id ")? <whitespace>* <queryParameter> <whitespace>* <options>?

<queryParameter> ::= <logicalOperatorExpression> (<whitespace>+ <queryParameter>)*
...
<singleComparison> ::=
    <FieldIdentifier> <whitespace>+ ( "=" | "!=" | ">"  | "<" | ">=" | "<=" | "starts" ) <whitespace>+ ( <NumericalLiteral> | <StringLiteral> | <FieldIdentifier> )
...
// select id idShort starts "some str"  --> put a "where" between "id" and "idShort"?
....
