...
1. Only selected attributes defined by the AAS data model xref:bibliography.adoc#bib1[[1\]] can be used in query filters: Usage of custom or vendor-specific attributes outside of the AAS specifications is generally not recommended and may lead to a rejections of the query by the receiving systems.
...
[.underline]#Constraint AASa-005:# Only the SubmodelElements root delaration can be followed with IdShortPaths.
...
| `$sme` | Starting point for fields available in Submodel Elements. Can be followed with an IdShortPath, see Clause AASa-005. Can start at any possible Submodel Element, is *not* restricted to Submodel Elements directly in the `Submodel/submodelElements` list. | `$sme.smeCollectionIdShort.propertyIdShort#value`, `$sme#value` to search recursively
| `$cd` | Starting point for fields available in Concept Descriptions | `$cd#id`
| `$aasdesc` | Starting point for fields available in Asset Administration Shell Descriptors | `$aasdesc#submodelDescriptors[0].endpoints[0].protocolinformation.href`
...
In case of an error the comparison result is invalid. Any further combinations with the invalid result will also have an invalid result. In that case the complete filter expression will be ignored for an element evaluated and no result will be returned for that element. Only the elements with valid results will be returned.
...
The `$match` operator signals that the following filter clauses (a) contain at least 1 list of elements with `[]` syntax, and that (b) all filters shall be evaluated on the same element of this list. The common path prefix left to a `[]` determines the list to be searched.
...
This JSON schema allows automatic code generation which has been validated with jsonschema2pojo. To allow such automatic code generation, polymorhism by the use of oneof in the JSON schema must be avoided. +
JSON schema allows to use regular expressions. Such regular expressions are used to validate the syntax of FieldIdentifiers. To be able to use common JSON schema validators, the depth and details have been limited in the corresponding regular expressions.
...
| strModel | - | string following the modelStringPattern
...
| filter | - | Root object for the query filter expression
ToHex> |
|===
...
{ +
  "filter": { +
    "$eq": [ +
      { "strModel": "$aas#idShort" }, +
      { "strModel": "$aas#assetInformation.assetType" } +
    ]
  }
}
|===
...
|
{
  "filter": { +
    "$match": [ +
      { "$eq": [ +
          { "strModel": "$sme.Documents[].DocumentClassification.Class#value" }, +
          { "strVal": "03-01" } +
        ]
      }, +
      { "$eq": [ +
          { "strModel": "$sme.Documents[].DocumentVersion.SMLLanguages[]#language" }, +
          { "strVal": "nl" } +
        ]
      }
    ]
...
$and( +
	$match( +
		...
		$sme.ProductClassifications.ProductClassificationItem.ProductClassId#value $eq "27-37-09-05" +
	), +
	$match( +
		$sm#idShort $eq "TechnicalData", +
  ...
  "filter": { +
    "$and": [ +
      { "$match": [ +
          ...
          { "$eq": [ +
              { "strModel": "$sme.ProductClassifications.ProductClassificationItem.ProductClassId#value" }, +
              { "strVal": "27-37-09-05" } +
            ]
          }
        ]
...
|
{
  "filter": { +
    "$or": [ +
      { "$match": [ +
          { "$eq": [ +
              { "strModel": "$aas#assetInformation.specificAssetIds[].name" }, +
              { "strVal": "supplierId" } +
            ]
          }, +
          { "$eq": [ +
              { "strModel": "$aas#assetInformation.specificAssetIds[].value" }, +
              { "strVal": "aas-1" } +
            ]
          }
        ]
