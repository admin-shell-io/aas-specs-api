...
If *_payload_* is mentioned as output parameter, only the returned payload in case of a successful operation (status code: Success, SuccessCreated) is denoted in column _Type_. In case of failure see xref:IDTA-01002_Spec_InterfacesPayload.adoc#StatusCode[Generic Status Codes].
