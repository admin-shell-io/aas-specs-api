...
====
Note: this is conformant to the change made in Part 1 V3.0 xref:IDTA-01002_Bibliography.adoc#bib1[[1\]].
====
...
AAS API versioning uses the major and minor version as described above. A specific AAS API version uses specific related versions of the metamodel as defined in Clause xref:IDTA-01002_Preamble.adoc#metamodel-versions[Metamodel Versions]. AAS API versions with the same major version must remain compatible, i.e. a client written for an older or a newer minor version must still work. This requires corresponding testing of clients and servers.
...
Pagination is a commonly used pattern to break down potentially long result lists into smaller pieces for a better control of the network and computational load on both the server and the client side. For instance, the OData protocol xref:IDTA-01002_Bibliography.adoc#bib8[[8\]] provides guidelines for parameters and behavior on the client and server side. In addition, the proposals of the RFC 8977 footnote:[see Chapter 2.4 of RFC 8977] present a best practice for web APIs. In the scope of the AAS HTTP/REST API, the query parameter “cursor” controls, which part of a longer result set is returned.
...
The payload is generated from the technology-neutral specification as described in Part 1 of the Asset Administration Shell Series for JSON xref:IDTA-01002_Bibliography.adoc#bib1[[1\]].

The serialization of JSON values is described in Clause 11.4.2.

Additional classes needed for payload of the HTTP/REST API specification can be found in xref:IDTA-01002_Spec_InterfacesPayload.adoc#metamodel-specification-details[Metamodel Specification Details].
...
|===
|*xref:IDTA-01002_Spec_InterfacesPayload.adoc#StatusCode[Generic status code]* |*Meaning* |*HTTP status code* h|Explanation
