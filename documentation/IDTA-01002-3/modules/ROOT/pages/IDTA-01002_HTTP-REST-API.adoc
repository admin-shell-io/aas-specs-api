...
* For Content=Value, the requested object is always serialized to an unnamed JSON Object or Array. This means that the response object must not have with the object’s idShort at the root level.
...
* The behavior of Level in combination with Content=Metadata is undefined. A server may ignore the Level modifier if it is set.
...
|PostSubmodelReference |POST |/aas/submodel-refs | Location header of the response contains the value ‘<baseUrl>/aas/submodel-ref/{submodelIdentifier}’ 
