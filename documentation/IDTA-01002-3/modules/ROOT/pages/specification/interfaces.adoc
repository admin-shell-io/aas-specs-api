...
The Serialization interface enables the export of one or more specified AAS or Submodels in different serialization formats. Concept descriptions may be included or omitted in the output.
...
The Query Interface is substantially different to the other Interfaces. Queries enable the sender to formulate its information request in own many different means, while the receiver has to react to that degree of freedom. The Query Language (see Clause [TODO: insert reference]) defines the content of the inputs. An additional difference is the option for senders to select the desired format of the response data object. The concrete mechanics are defined in the technology-specific sections, e.g. Clause (see Clause [TODO: insert reference for the HTTP Query API]).
...
2+|Interface: Query
h|Operation Name h|Description
e|QueryAssetAdministrationShells | Returns a list of Asset Administration Shells based on an input query.
e|QuerySubmodels | Returns a list of Submodels based on an input query.
e|QueryAssetAdministrationShellDescriptorsByQuery | Returns a list of Asset Administration Shell Descriptors based on an input query.
e|QuerySubmodelDescriptors | Returns a list of Submodel Descriptors based on an input query.
e|QueryConceptDescriptions | Returns a list of Concept Descriptions based on an input query.
|===
