...
h|Operation  h|Kind of Change h|Comment
 ...
 | GetAllAssetAdministrationShells | change a| added new input parameters `createdFrom` and `updatedFrom` for recent changes as well as `key` and `keyIdentifier` for specific Asset IDs.
 | GetAllSubmodels  | change a| added new input parameters `createdFrom` and `updatedFrom` for recent changes as well as `semanticId`
