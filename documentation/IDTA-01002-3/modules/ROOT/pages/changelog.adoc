...
h|Operation  h|Kind of Change h|Comment
 ...
 | GetAllAssetAdministrationShellDescriptors | change a| added new input parameters `createdFrom` and `updatedFrom` 
 | GetAllSubmodelDescriptors | change a| added new input parameters `createdFrom` and `updatedFrom` for recent changes as well as `key` and `keyIdentifier` for specific Asset IDs.
