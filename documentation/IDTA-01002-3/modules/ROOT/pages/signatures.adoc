...
An example is a device manufaturer supplying to an integrator supplying to a plant operator. The plant operator wants to check the remained integrity of device manufacturer's AAS.
The AASX package format includes the possibility of signing an AASX package, but this is seldomly used. AASX packages can also not be protected by access rules. This is why signatures are needed as part of the API.
