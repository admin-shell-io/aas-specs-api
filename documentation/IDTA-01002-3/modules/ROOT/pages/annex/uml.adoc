...
<<class>> shows a class with name "Class1" and an attribute with name "attr" of type _Class2_.
Attributes are owned by the class.
Some of these attributes may represent the end of binary associations, see also <<attribute-notation>>.
...
Association ends owned by classes are always navigable, while those owned by associations may be navigable or not. xref:IDTA-01002_Bibliography.adoc#bib35[[35\]]”]

.Class
[[class]]
image::uml/class.png[]

<<inheritance>> shows that _Class4_ inherits all member elements from _Class3_.
...
<<multiplicity>> defines the required and allowed multiplicity/cardinality within an association between instances of _Class1_ and _Class2_.
...
The default is a set, i.e. it is not ordered and the elements within the collection are unique and contain no duplicates. <<ordered-multiplicity>> shows an ordered collection: the instances of _Class2_ related to an instance of _Class1_.
...
<<association>> shows that the member ends of an association can be named as well, i.e. an instance of _Class1_ can be in relationship "relation" to an instance of _Class2_.
...
<<composition>> shows a composition, also called a composite aggregation.
...
There is no instance of _Class2_ without a relationship to an instance of _Class1_. <<composition>> shows the composition using an association relationship with a filled diamond as composition adornment.

.Composition (Composite Aggregation)
[[composition]]
image::uml/composition.png[]

<<aggregation>> shows an aggregation.
An aggregation is a binary association.
In contrast to a composition, an instance of _Class2_ can be shared by several instances of _Class1_. <<aggregation>> shows the shared aggregation using an association relationship with a hallow diamond as aggregation adornment.

.Aggregation
[[aggregation]]
image::uml/aggregation.png[]

<<attribute-notation>> illustrates that the attribute notation can be used for an association end owned by a class.
...
<<default-value>> shows a class with three attributes with primitive types and default values.
...
<<dependency>> shows that there is a dependency relationship between _Class1_ and _Class2_.
...
<<abstract>> shows an abstract class.
...
<<package>> shows a package named "Package2".
...
<<imported-package>> shows that all elements in _Package2_ are imported into the namespace defined by _Package1_.
...
<<enumeration>> shows an enumeration with the name "Enumeration1".
...
////
<<primitive>> shows a primitive data type with the name "int".
...
<<graphical-representation-composite-aggregation>> shows different graphical representations of a composition (composite aggregation).
...
<<graphical-representation-shared-aggregation>> shows different representations of a shared aggregation: a _Class2_ instance can exist independently of a _Class1_ instance; it only references the instances of _Class2_.
...
A specialty in <<graphical-representation-shared-aggregation>> is that the aggregated instances are referables in the sense of the Asset Administration Shell metamodel (i.e. they inherit from the predefined abstract class "Referable").
...
<<graphical-representation-inheritance>> show different graphical representations of generalization.
...
<<graphical-representation-enumeration-with-inheritance>> depicts different graphical notations for enumerations in combination with inheritance.
...
This can be seen at the right side of <<graphical-representation-enumeration-with-inheritance>> that defines the same enumeration but without inheritance.
====
...
<<graphical-representation-template>> shows a class representing a template.
