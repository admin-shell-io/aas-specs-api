...
e|Deep (Default) a|All elements of a requested hierarchy level and all children on all sublevels are returned. Children in this sense are SubmodelElements which are contained at the ‘submodelElements’ field of Submodels, the ‘value’ field of SubmodelElementCollections or SubmodelElementLists, the ‘statements’ field of Entities, or the ‘annotations’ field of AnnotatedRelationshipElements.
e|Core a|Only elements of a requested hierarchy level as well as direct children are returned. By this, a client can iterate the hierarchy step by step.
|===


====
Note: level parameters are mapped to the query parameter “?level” in the HTTP/REST APIs, see also xref:IDTA-01002_HTTP-REST-API.adoc#modifier-constraints[Modifier Constraints].
====
...
====
Note: level parameters are mapped to path suffixes “/$<content>” in the HTTP/REST APIs, see also xref:IDTA-01002_HTTP-REST-API.adoc#modifier-constraints[Modifier Constraints].
====
...
====
Note: level parameters are mapped to the query parameter “?extent” in the HTTP/REST APIs, see also xref:IDTA-01002_HTTP-REST-API.adoc#modifier-constraints[Modifier Constraints].
====
