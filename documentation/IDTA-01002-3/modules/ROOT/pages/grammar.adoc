...
<sortExpression> ::= "$sort" <ws> "(" <ws> "id" <ws> ( "asc" | "dsc" ) <ws> ")" <ws>
<limitExpression> ::= "$limit" <ws> "(" <ws> <NumericalLiteral> <ws> "," <ws> <NumericalLiteral> <ws> ")" <ws>
...
<FieldIdentifierAasDescriptor> ::= "$aasdesc#" ( "idShort" | "id" | "assetKind" | "assetType" | "globalAssetId" | <SpecificAssetIdsClause> | "submodelDescriptors" ( "[" ( [0-9]* ) "]" )? "." <SmDescriptorClause> ) <ws>
